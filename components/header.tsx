      ...
      <>
              ...
              <ul style={navStyles} className={`${nav.navTypeStyle} ${nav.navAlignment} w-48 flex-grow list-none md:hidden pointer-events-auto`}>
                ...
                {navItems.map(function (item, index) {
                  ...
                  if (subNavItems) {
                    ...
                    return (
                      <li className="" key={index}>
                        <a style={linkStyles} className={"block no-underline hover:underline hover:text-primary"} href={item.link} target={linkTarget(item.link)}>{item.label}</a>
                      </li>
                    )
                  }
                })}
        ...
        <div className={`${sectionClasses} hidden md:block h-screen fixed z-40 top-0 left-0 right-0`}>
          ...
          <div className={`w-full md:p-5`}>
            <Logo className="absolute top-4 left-4 w-16" />
