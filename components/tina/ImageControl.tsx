...
const FieldRow = ({ inputValue='', onUpdate=(value)=>{ value }, isMobile = false }) => {
  ...
  const positionOptions = buildOptions(positions, isMobile)
  ...
  const [position, setPosition] = useState(getStyleMatch(positionOptions, inputValue));
  const [margin, setMargin] = useState(getStyleMatch(marginOptions, inputValue));

  useEffect(() => {
    onUpdate(`${height} ${fit} ${position} ${margin}`)
  }, [height, fit, position, margin]);
